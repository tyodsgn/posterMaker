package com.tyodsgn.postermaker

import android.os.Bundle
import androidx.activity.ComponentActivity
import androidx.activity.compose.setContent
import androidx.activity.enableEdgeToEdge
import androidx.compose.animation.slideInHorizontally
import androidx.compose.animation.slideOutHorizontally
import androidx.compose.foundation.layout.padding
import androidx.compose.material3.ExperimentalMaterial3Api
import androidx.compose.material3.Scaffold
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.tooling.preview.Preview
import androidx.navigation.compose.NavHost
import androidx.navigation.compose.composable
import androidx.navigation.compose.rememberNavController
import com.tyodsgn.postermaker.ui.theme.PosterMakerTheme

class MainActivity : ComponentActivity() {
    @OptIn(ExperimentalMaterial3Api::class)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContent {
            PosterMakerTheme {
                val navController = rememberNavController()
                Scaffold {
                    val padd = it
                    NavHost(
                        navController = navController,
                        startDestination = "productDetailScreen",
                        enterTransition = { slideInHorizontally(initialOffsetX = { it }) },
                        exitTransition = { slideOutHorizontally(targetOffsetX = { -it }) },
                        popEnterTransition = { slideInHorizontally(initialOffsetX = { -it }) },
                        popExitTransition = { slideOutHorizontally(targetOffsetX = { it }) },
                        sizeTransform = null
                    ){
                        composable("posterHomeScreen"){
                            PosterHomeScreen(navController = navController)
                        }
                        composable("imageEditorScreen/{name}"){
                            val imageName = it.arguments?.getString("name")
                            ImageEditorScreen(
                                navController = navController,
                                imageName = imageName!!,
                                modiferTopAppBar = Modifier
                                    .padding(top = padd.calculateTopPadding())
                            )
                        }

                        composable("productDetailScreen") {
                            ProductDetailScreen(
                                modiferTopAppBar = Modifier
                                    .padding(top = padd.calculateTopPadding()),
                                navController = navController
                            )
                        }
                    }
                }

            }
        }
    }
}

@Composable
fun Greeting(name: String, modifier: Modifier = Modifier) {
    Text(
        text = "Hello $name!",
        modifier = modifier
    )
}

@Preview(showBackground = true)
@Composable
fun GreetingPreview() {

    PosterMakerTheme {
        Greeting("Android")
    }
}